...
//	JBDump($field->type, 0);
	switch ($field->type) {
		...
		case 'field_x':
			...
			// FieldX buttons
			$html	 = preg_replace('/(<(div)([^>]*)class="([^"]*)(collection-group-button)([^"]*)"[^>]*>)/u', '<div$3class="$5 uk-width-auto uk-iconnav">', $html);
			...
			$html	 = preg_replace('/(<div[^>]*class="([^"]*)(button-drag)([^"]*)"[^>]*><(span)[^>]*class="([^"]*)(icon-circle)([^"]*)"[^>]*><\/span><\/div>)/u', '<div><a href="#" onclick="return false;" uk-icon="icon: move" class="uk-text-primary button-drag"></a></div>', $html);
			$html	 = preg_replace('/class=\\"([^\\"]*)(uk-form-controls)([^\\"]*)\\"/', 'class="$1 uk-margin-remove-left $3"', $html);
			$html	 = preg_replace('/class=\\"([^\\"]*)(cck_form_group_x)([^\\"]*)\\"/', 'class="$1 uk-clearfix uk-margin $3"', $html);
			$html	 = preg_replace('/class=\\"([^\\"]*)(cck_wysiwyg_editor)([^\\"]*)\\"/', 'class="$1 uk-clearfix uk-margin1 $3"', $html);
